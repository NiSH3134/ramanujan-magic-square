...
export default function CustomDatePicker({ selectedDate, onDateChange }: CustomDatePickerProps) {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (date) {
      ...
      setIsOpen(false);
      if (onDateChange) onDateChange(undefined as any);
    }
  };
        ...
        <div className="absolute z-50 mt-2 w-full bg-[var(--color-primary)] text-[var(--color-secondary)] shadow-[var(--shadow)] rounded-lg">
          ...
          <DayPicker
            ...
            classNames={{
              ...
              today: 'bg-[var(--color-tertiary)] border-[var(--color-accent)] text-[var(--color-primary)] rounded-md',
              selected: 'bg-[var(--color-accent)] text-[var(--color-primary)] border-[var(--color-accent)] rounded-md ',
            }}
          />
